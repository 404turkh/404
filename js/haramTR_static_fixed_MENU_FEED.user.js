...
function startAutoFeed() {
    ...
    feedInterval = setInterval(() => {
        ...
        bots.forEach(bot => {
            ...
            // Feed (W)
            bot.send(new Uint8Array([21]));

            // Feed sonrası çık + respawn
            setTimeout(() => {
                try {
                    bot.ws.close();
                    setTimeout(() => {
                        if (typeof spawnBot === "function") {
                            spawnBot(bot);
                            bot.mass = START_MASS;
                        }
                    }, 300);
...
// Menüye buton ekleme
function addFeedMenuButton() {
    const menu = document.querySelector("#menu") || document.body;
...
setTimeout(addFeedMenuButton, 2000);
// ===== BİTTİ =====
