...
function startAutoFeed() {
    ...
    feedInterval = setInterval(() => {
        ...
        bots.forEach(bot => {
            ...
            bot.send(new Uint8Array([21])); // W feed
            setTimeout(() => {
                try {
                    bot.ws.close();
                    setTimeout(() => {
...
function addFeedButtonToTopMenu() {
    const buttons = [...document.querySelectorAll("button")];
    const refBtn = buttons.find(b => b.innerText.includes("AI"));
    if (!refBtn || refBtn.dataset.feedAdded) return;
    ...
    refBtn.dataset.feedAdded = "1";
}

setInterval(addFeedButtonToTopMenu, 1000);
// ===== BİTTİ =====
