...
(function() {
    ...
    const initUI = () => {
        ...
        const panelHTML = `
            ...
            <div class="harambro-info-panel">
                ...
                <button class="bot-button harambro-stfinish" onclick="window.startBots('stfinish')">⏻ Start</button>
                <button class="bot-button harambro-stop" onclick="window.startBots('stop')" style="display: none">⏸ Stop</button>
                ...
                <button class="small-button" onclick="window.toggleFixed()">📍 Sabit OFF</button>
    ...
    window.toggleFixed = () => {
        ...
        if (btn) {
            btn.textContent = `📍 Sabit ${fixedMode ? 'ON' : 'OFF'}`;
        }
...
/* ==== HARMBRO UI STABILITY PATCH (non-breaking) ==== */
(function(){
 if (window.__harambro_ui_patch) return;
 window.__harambro_ui_patch = true;
   ...
   try{
     if (window.botConfig){
       document.querySelectorAll("button").forEach(b=>{
         if(b.textContent.includes("Ai")){
           b.textContent = "Ai " + (window.botConfig.botAi ? "ON":"OFF");
         }
         if(b.textContent.includes("Sabit")){
           b.textContent = "📍 Sabit " + (window.fixedMode ? "ON":"OFF");
         }
       });
     }
   }catch(e){}
 ...
 document.addEventListener("click",function(e){
   ...
   if(b.classList.contains("harambro-stfinish")){
     safe(()=>window.startBots && window.startBots("stfinish"));
   }
   if(b.textContent.includes("Ai")){
     safe(()=>window.toggleAIMode && window.toggleAIMode());
     setTimeout(updateLabels,50);
   }
   if(b.textContent.includes("Sabit")){
     safe(()=>window.toggleFixed && window.toggleFixed());
     setTimeout(updateLabels,50);
   }
 },true);
