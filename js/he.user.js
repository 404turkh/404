...
(function() {
    ...
    // === OBFUSCATION & ANTI-TAMPER BLOĞU ===
    const decodeB64 = (str) => atob(str);

    const OBF = {
        // Panelde gözüken isim "@5harambro"
        ...
        // Botların default ismi "5harambro"
        defaultName: "NWhhcmFtYnJv"
    };

    const OBF_CHECKSUM = 20233; // OBF içeriği değişirse bu tutmaz ve script kapanır.

    function verifyIntegrity() {
        const vals = Object.values(OBF).join("|");
        let sum = 0;
        const MOD = 1000000007;
        for (let i = 0; i < vals.length; i++) {
            sum = (sum + vals.charCodeAt(i)) % MOD;
        }
        if (sum !== OBF_CHECKSUM) {
            console.error("Integrity check failed – script disabled.");
            return false;
        }
        return true;
    }

    // Biri OBF değerlerini kurcalarsa, buradan sonra script ÇALIŞMAZ.
    if (!verifyIntegrity()) {
        return;
    }
    ...
    class Bot {
        ...
        handleBuffer(buffer) {
            ...
            switch (reader.readUint8()) {
                case 32:
                    ...
                    // === AUTO MASS 130: spawn sonrası otomatik feed ===
        ...
        // Spawn sonrası otomatik 130 mass civarı feed
        startAutoMassFeed() {
            if (this.autoFeedInterval) {
                clearInterval(this.autoFeedInterval);
                this.autoFeedInterval = null;
            ...
            this.autoFeedInterval = setInterval(() => {
                ...
                this.eject(); // W
                feedCount++;
                // 18 feed ≈ 120–140 mass arası, hoşuna göre arttır/azalt
                if (feedCount >= 18) {
                    clearInterval(this.autoFeedInterval);
                    this.autoFeedInterval = null;
                }
    ...
    const botConfig = {
        ...
        startedBots: false,
        vShield: false,
        autoMass130: false, // ✅ Yeni özellik: spawn sonrası otomatik 130 mass
    ...
    const initUI = () => {
        ...
        if (titleEl) {
            titleEl.textContent = decodeB64(OBF.title); // "@5harambro"
        }
    ...
    function createSettingsModal() {
        ...
        // Bot name input doldurma (gizli default + localStorage)
        const nameInput = modal.querySelector("#botName");
        if (nameInput) {
            const saved = localStorage.getItem('harambro-bot-name');
            const defTitle = decodeB64(OBF.title); // "@5harambro"
    ...
    function saveSettings() {
        ...
        const defaultTitle = decodeB64(OBF.title); // "@5harambro"
    ...
    // Eski static fonksiyonları (UI'da buton yok ama istersen kullanırsın)
    window.toggleFixed = () => {
        fixedMode = !fixedMode;
