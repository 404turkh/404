...
function shouldShowAdNow() {
  const now = Date.now();
  const last = Number(localStorage.getItem("lastAdTime") || 0);
  return !last || (now - last) > AD_INTERVAL;
}

function markAdShown() {
  localStorage.setItem("lastAdTime", String(Date.now()));
}
...
function openVerifyModal(targetUrl) {
  ...
  style.textContent = `
    #__verify_gate__ .vg-box{position:relative;overflow:hidden;width:min(520px,calc(100vw - 36px));border-radius:22px;background:rgba(18,18,22,0.92);border:1px solid rgba(255,255,255,0.10);box-shadow:0 22px 70px rgba(0,0,0,0.55);padding:18px;color:#fff;font:600 14px -apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Arial;transform:translateY(10px) scale(.985);animation:vgPop .18s ease-out forwards;}
    #__verify_gate__ .vg-glow{position:absolute;inset:-2px;background:radial-gradient(600px 240px at 18% 0%, rgba(34,197,94,.18), transparent 60%),radial-gradient(520px 220px at 100% 22%, rgba(34,211,238,.14), transparent 55%),radial-gradient(420px 220px at 0% 100%, rgba(225,48,108,.12), transparent 55%);pointer-events:none;}
    #__verify_gate__ .vg-head{display:flex;align-items:center;gap:12px;margin-bottom:10px;position:relative;z-index:1;}
    #__verify_gate__ .vg-lock{width:40px;height:40px;border-radius:14px;display:grid;place-items:center;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.10);}
    ...
    #__verify_gate__ .vg-progressTop{display:flex;align-items:center;justify-content:space-between;font-weight:800;opacity:.9;margin-bottom:8px;}
    #__verify_gate__ .vg-bar{height:10px;border-radius:999px;background:rgba(255,255,255,0.10);border:1px solid rgba(255,255,255,0.10);overflow:hidden;}
    #__verify_gate__ .vg-bar>div{height:100%;width:0%;border-radius:999px;background:linear-gradient(90deg, rgba(34,197,94,0.9), rgba(34,211,238,0.7));transition:width .22s ease;}
    #__verify_gate__ .vg-steps{display:flex;gap:10px;margin-top:10px;opacity:.95;font-weight:800;}
    #__verify_gate__ .vg-dot{width:10px;height:10px;border-radius:50%;background:rgba(255,255,255,0.22);border:1px solid rgba(255,255,255,0.12);}
    #__verify_gate__ .vg-dot.done{background:rgba(34,197,94,0.85);border-color:rgba(34,197,94,0.55);box-shadow:0 0 0 4px rgba(34,197,94,0.10);}
    #__verify_gate__ .vg-btn{width:100%;display:flex;align-items:center;justify-content:space-between;gap:10px;padding:12px 12px;border-radius:16px;border:1px solid rgba(255,255,255,0.12);background:rgba(255,255,255,0.08);color:#fff;font-weight:900;cursor:pointer;user-select:none;transition:transform .08s ease, background .16s ease, border-color .16s ease;position:relative;z-index:1;margin-bottom:10px;}
    #__verify_gate__ .vg-btn:active{transform:scale(.99);}
    #__verify_gate__ .vg-left{display:flex;align-items:center;gap:10px;}
    #__verify_gate__ .vg-ico{width:34px;height:34px;border-radius:12px;display:grid;place-items:center;border:1px solid rgba(255,255,255,0.10);background:rgba(255,255,255,0.07);}
    #__verify_gate__ .vg-badge{padding:6px 10px;border-radius:999px;border:1px solid rgba(255,255,255,0.14);background:rgba(255,255,255,0.08);font-weight:900;opacity:.92;white-space:nowrap;}
    ...
    #__verify_gate__ .vg-btn.done{background:rgba(34,197,94,0.16)!important;border-color:rgba(34,197,94,0.30)!important;}
    #__verify_gate__ .vg-btn.done .vg-badge{border-color:rgba(34,197,94,0.35);background:rgba(34,197,94,0.14);}
    #__verify_gate__ .vg-status{margin:2px 0 12px 0;font-weight:850;opacity:.9;position:relative;z-index:1;}
    #__verify_gate__ .vg-actions{display:flex;gap:10px;position:relative;z-index:1;margin-top:6px;}
    #__verify_gate__ .vg-cancel,#__verify_gate__ .vg-unlock{flex:1;padding:11px 12px;border-radius:14px;border:1px solid rgba(255,255,255,0.12);font-weight:950;cursor:pointer;user-select:none;transition:transform .08s ease, opacity .16s ease;}
    #__verify_gate__ .vg-cancel{background:rgba(255,255,255,0.08);color:#fff;}
    #__verify_gate__ .vg-unlock{background:linear-gradient(90deg, rgba(34,197,94,0.95), rgba(34,211,238,0.80));color:#07120a;border-color:rgba(255,255,255,0.14);}
    #__verify_gate__ .vg-unlock:disabled{opacity:.45;cursor:not-allowed;filter:grayscale(.15);}
    #__verify_gate__ .vg-cancel:active,#__verify_gate__ .vg-unlock:active{transform:scale(.99);}
    @keyframes vgPop{to{transform:translateY(0) scale(1);}}
  `;
...
const data = {
  ...
  list2: [
    ...
    {logo:"gnise.png",name:"ESign ✅",desc:"XL AXIATA, PT TBK",url:"https://api.khoindvn.io.vn/fxHqSE"},
    {logo:"gnise.png",name:"ESign",desc:"BOC International (China) Limited",url:"https://api.khoindvn.io.vn/nbSQ2q"},
    {logo:"gnise.png",name:"ESign BOC V1",desc:"BOC International (China) Limited",url:"https://api.khoindvn.io.vn/Fv5vFZ"},
    {logo:"gnise.png",name:"ESign BOC V2",desc:"BOC International (China) Limited",url:"https://api.khoindvn.io.vn/FLwKeO"},
    {logo:"gnise.png",name:"ESign BOC V3",desc:"BOC International (China) Limited",url:"https://api.khoindvn.io.vn/Lxiubx"},
    {logo:"gnise.png",name:"ESign",desc:"National Oilwell Varco, Inc.",url:"https://api.khoindvn.io.vn/SiRDpZ"},
    {logo:"gnise.png",name:"ESign",desc:"VIETNAM AIRLINES JSC",url:"https://api.khoindvn.io.vn/IosVVa"},
    {logo:"gnise.png",name:"ESign",desc:"PowerChina International Group Limited",url:"https://api.khoindvn.io.vn/VlLV6l"},
    {logo:"gnise.png",name:"ESign PowerChina V1",desc:"PowerChina International Group Limited",url:"https://api.khoindvn.io.vn/7944EP"},
    {logo:"gnise.png",name:"ESign PowerChina V2",desc:"PowerChina International Group Limited",url:"https://api.khoindvn.io.vn/btX260"},
    {logo:"gnise.png",name:"ESign PowerChina V3",desc:"PowerChina International Group Limited",url:"https://api.khoindvn.io.vn/cDNdxx"},
    {logo:"gnise.png",name:"ESign PowerChina V4",desc:"PowerChina International Group Limited",url:"https://api.khoindvn.io.vn/mobNoQ"},
    {logo:"gnise.png",name:"ESign",desc:"Qingdao Rural Commercial Bank Co., Ltd",url:"https://api.khoindvn.io.vn/JHqEjv"},
    {logo:"gnise.png",name:"ESign",desc:"Commission on Elections",url:"https://api.khoindvn.io.vn/CkOyq3"},
    {logo:"gnise.png",name:"ESign Com V1",desc:"Commission on Elections",url:"https://api.khoindvn.io.vn/6UMBaC"},
    {logo:"gnise.png",name:"ESign Com V2",desc:"Commission on Elections",url:"https://api.khoindvn.io.vn/GX5cq7"},
    {logo:"gnise.png",name:"ESign",desc:"Luoyang Postal Administration",url:"https://api.khoindvn.io.vn/m3iXgW"},
    {logo:"gnise.png",name:"ESign",desc:"CHIBA INSTITUTE OF TECHNOLOGY",url:"https://api.khoindvn.io.vn/8oAYgJ"},
    {logo:"gnise.png",name:"ESign ChiBa V1",desc:"CHIBA INSTITUTE OF TECHNOLOGY",url:"https://api.khoindvn.io.vn/OtZYJW"},
    {logo:"gnise.png",name:"ESign ChiBa V2",desc:"CHIBA INSTITUTE OF TECHNOLOGY",url:"https://api.khoindvn.io.vn/qeSTpu"},
    {logo:"gnise.png",name:"ESign ChiBa V3",desc:"CHIBA INSTITUTE OF TECHNOLOGY",url:"https://api.khoindvn.io.vn/AhPiWP"},
    {logo:"gnise.png",name:"ESign ChiBa V4",desc:"CHIBA INSTITUTE OF TECHNOLOGY",url:"https://api.khoindvn.io.vn/1UIfSe"},
    {logo:"gnise.png",name:"ESign",desc:"VIETNAM JOINT STOCK COMMERCIAL BANK FOR INDUSTRY AND TRADE",url:"https://api.khoindvn.io.vn/axb5B2"},
    {logo:"gnise.png",name:"ESign VN V1",desc:"VIETNAM JOINT STOCK COMMERCIAL BANK FOR INDUSTRY AND TRADE",url:"https://api.khoindvn.io.vn/3t0t8X"},
    {logo:"gnise.png",name:"ESign VN V2",desc:"VIETNAM JOINT STOCK COMMERCIAL BANK FOR INDUSTRY AND TRADE",url:"https://api.khoindvn.io.vn/yA7Vr0"},
    {logo:"gnise.png",name:"ESign",desc:"GLOBAL TAKEOFF, INC",url:"https://api.khoindvn.io.vn/4D8wjF"},
    {logo:"gnise.png",name:"ESign GLOBAL V1",desc:"GLOBAL TAKEOFF, INC",url:"https://api.khoindvn.io.vn/hZgAt7"},
    {logo:"gnise.png",name:"ESign GLOBAL V2",desc:"GLOBAL TAKEOFF, INC",url:"https://api.khoindvn.io.vn/fnOXAo"},
    {logo:"gnise.png",name:"ESign GLOBAL V3",desc:"GLOBAL TAKEOFF, INC",url:"https://api.khoindvn.io.vn/BEZcKD"},
    {logo:"gnise.png",name:"ESign GLOBAL V4",desc:"GLOBAL TAKEOFF, INC",url:"https://api.khoindvn.io.vn/skjKPF"},
    {logo:"gnise.png",name:"ESign GLOBAL V5",desc:"GLOBAL TAKEOFF, INC",url:"https://api.khoindvn.io.vn/VjHmnf"},
    {logo:"gnise.png",name:"ESign GLOBAL V6",desc:"GLOBAL TAKEOFF, INC",url:"https://api.khoindvn.io.vn/ufbyRf"}
  ],
  list3: [
