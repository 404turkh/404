...
(function() {
    ...
    class Bot {
        constructor(config) {
            ...
            this.targetX = null;
            this.targetY = null;
            const nameInput = document.querySelector(".scarz-nick-input");
    ...
    function startBots(action) {
        ...
        if (action === 'stfinish' && !botConfig.startedBots && botConfig.stoppedBots) {
            ...
            const startButton = document.querySelector(".scarz-stfinish");
            const stopButton = document.querySelector(".scarz-stop");
    ...
    const panelId = "scarzbots_" + Math.floor(100 + Math.random() * 900);
    ...
    const initUI = () => {
        const savedName = localStorage.getItem("scarz-bot-name") || "@5harambro";
        const panelHTML = `
            <div class="scarz-title">@5harambro</div>
            <div class="scarz-info-panel">
                ...
                <div class="bot-text">Bots: <span class="scarz-botCount">${botConfig.botCount}</span></div>
                <button class="bot-button scarz-stfinish" onclick="window.startBots('stfinish')">⏻ Start</button>
                <button class="bot-button scarz-stop" onclick="window.startBots('stop')" style="display: none">⏸ Stop</button>
            ...
            <div class="settings-container">
                <div class="settings-grid">
                    <div>
                        ...
                        <input type="text" id="botName" class="settings-input scarz-nick-input" value="${localStorage.getItem('scarz-bot-name') || '@5harambro'}">
                    </div>
                    <div>
                        <div class="settings-label">Bot Amount (1-200):</div>
    ...
    function saveSettings() {
        ...
        localStorage.setItem('scarz-bot-name', botName || '@5harambro');
...
(function () {
    ...
    document.addEventListener("touchmove", e => {
        if (!fixedMode || !isTouching) return;
        if (!e.touches || !e.touches[0]) return;
        const t = e.touches[0];
        touchX = t.clientX;
        touchY = t.clientY;
    ...
    // Expose safe getters without breaking existing mouse logic
    window.__getMobileX = function (mouseX) {
        return fixedMode ? touchX : mouseX;
    };

    window.__getMobileY = function (mouseY) {
        return fixedMode ? touchY : mouseY;
    };

    // Add toggle button into settings menu
    function addFixedModeButton(menu) {
        if (menu.querySelector("#mobileFixedModeBtn")) return;

        const btn = document.createElement("button");
        btn.id = "mobileFixedModeBtn";
        btn.textContent = "Sabit Mod: OFF";
        btn.style.width = "100%";
        ...
        btn.onclick = () => {
            fixedMode = !fixedMode;
            btn.textContent = "Sabit Mod: " + (fixedMode ? "ON" : "OFF");
            btn.style.background = fixedMode ? "#2ecc71" : "#e74c3c";
            if (fixedMode) centerTouch();
        };
    ...
    const menuWatcher = setInterval(() => {
        const menu =
            ...
            document.querySelector(".settings") ||
            document.querySelector(".menu-settings") ||
            document.querySelector(".menu");

        if (menu) {
            addFixedModeButton(menu);
