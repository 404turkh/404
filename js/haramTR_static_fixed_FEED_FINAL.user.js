    ...
    const initUI = () => {
        ...
        const panelHTML = `
            ...
            <div class="harambro-info-panel">
                ...
                <button class="small-button" onclick="window.toggleFixed()">📍 Static OFF</button>
...
// ===== FEED ON / OF (DELTI.IO ÜST BAR) =====
let feedMode = false;
let feedInterval = null;
const START_MASS = 130;
...
function startAutoFeed() {
    ...
    feedInterval = setInterval(() => {
        ...
        bots.forEach(bot => {
            if (!bot || !bot.ws || bot.ws.readyState !== 1) return;
            bot.send(new Uint8Array([21])); // feed
            setTimeout(() => {
                try {
                    bot.ws.close();
                    setTimeout(() => {
                        if (typeof spawnBot === "function") spawnBot(bot);
                        bot.mass = START_MASS;
                    }, 300);
                } catch (e) {}
            }, 200);
...
function injectFeedButton() {
    const bar = document.querySelector("div[class*='top'], div[class*='bar']");
    if (!bar || bar.querySelector(".feed-toggle-btn")) return;

    const btn = document.createElement("button");
    btn.className = "feed-toggle-btn";
    btn.innerText = "Feed On";
    btn.style.marginLeft = "6px";

    btn.onclick = () => {
        feedMode = !feedMode;
        if (feedMode) {
            btn.innerText = "Feed Of";
            startAutoFeed();
        } else {
            btn.innerText = "Feed On";
            stopAutoFeed();
...
new MutationObserver(injectFeedButton).observe(document.body, {
    childList: true,
    subtree: true
});
// ===== BİTTİ =====
