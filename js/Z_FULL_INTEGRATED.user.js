...
/* ===============================
   HARAMBRO ULTIMATE UI INTEGRATION
   Drag + Toggle + Real Counter
   Minimize to Custom Icon
================================= */

(function(){

if (window.__harambroUltimateIntegrated) return;
window.__harambroUltimateIntegrated = true;

const ICON_URL = "https://raw.githubusercontent.com/404turkh/404/main/icon.png";
...
window.toggleBots = function(){
    ...
    if (!botConfig.startedBots){
        ...
        window.startBots && window.startBots('stfinish');
        ...
        window.startBots && window.startBots('stop');
...
function startRealCounter(){
    ...
    counterInterval = setInterval(() => {
        if (window.Bots){
            ...
            const el = document.querySelector(".harambro-botCount");
            if(el) el.textContent = connected;
        }
    }, 500);
}

function stopRealCounter(){
    if(counterInterval){
...
/* ---------------- MINIMIZE TO ICON ---------------- */
function createMinimizeSystem(){

    const panel = document.querySelector(".harambro-info-panel");
    if (!panel) return;
    ...
    icon.style.boxShadow = "0 0 15px rgba(0,0,0,0.5)";
...
setTimeout(createMinimizeSystem, 2000);
